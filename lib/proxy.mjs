...
export const DEFAULT_MODELS = [
  ...
  { id: "GigaChat-3-Lightning", name: "GigaChat 3 Lightning" },
  { id: "GigaChat-2-Max", name: "GigaChat 2 Max" },
  { id: "GigaChat-2-Pro", name: "GigaChat 2 Pro" },
  { id: "GigaChat-2", name: "GigaChat 2" }
];

/** Known display names, used when /v1/models passthrough meets an unconfigured id. */
...
/**
 * Recursively ensure every object schema node carries `properties`. GigaChat's
 * legacy function-calling validation rejects object parameters without it
 * ("Field 'properties.args.properties' is missing") on the lighter models
 * (GigaChat-3-Lightning, GigaChat-2*), even though Ultra/Pro tolerate them.
 * Mutates in place; `body` is already a deep copy.
 */
function sanitizeSchemaNode(node, depth = 0) {
  if (depth > 16 || !node || typeof node !== "object" || Array.isArray(node)) return;
  if (node.type === "object" && node.properties === void 0) node.properties = {};
  if (node.properties && typeof node.properties === "object" && !Array.isArray(node.properties)) {
    for (const child of Object.values(node.properties)) sanitizeSchemaNode(child, depth + 1);
  }
  if (node.items && typeof node.items === "object") sanitizeSchemaNode(node.items, depth + 1);
  for (const key of ["anyOf", "oneOf"]) {
    if (Array.isArray(node[key])) for (const child of node[key]) sanitizeSchemaNode(child, depth + 1);
  }
}
...
export function translateRequest(body) {
  ...
  if (Array.isArray(b.tools) && b.tools.length > 0) {
    ...
    for (const f of b.functions) {
      if (f && f.parameters && typeof f.parameters === "object") sanitizeSchemaNode(f.parameters);
    }
...
/**
 * Human-readable error body for failed upstream calls (4xx/5xx). GigaChat
 * often answers with an empty body (e.g. 422), which surfaces in the chat UI
 * as a bare "422 status code (no body)" — replace that with actionable text
 * in the standard OpenAI error shape.
 */
function friendlyUpstreamError(status, text) {
  const trimmed = (text ?? "").trim();
  if (trimmed.length > 0) {
    let payload;
    try { payload = JSON.parse(trimmed); } catch { payload = void 0; }
    if (payload && typeof payload === "object" && payload.error !== void 0) {
      return { kind: "raw", status, raw: Buffer.from(trimmed), contentType: "application/json" };
    }
    const message = typeof payload?.message === "string" && payload.message.length > 0
      ? payload.message
      : trimmed.slice(0, 500);
    return { kind: "raw", status, raw: Buffer.from(JSON.stringify({ error: { message, type: "upstream_error", code: status } })), contentType: "application/json" };
  }
  const hints = {
    400: "GigaChat rejected the request (400).",
    401: "GigaChat rejected the access token (401), even after a refresh attempt — check the authorization key (base64(client_id:client_secret)) and the scope.",
    403: "GigaChat denied access (403): the account or scope likely lacks access to the requested model.",
    404: "GigaChat endpoint or model not found (404).",
    413: "Request too large for GigaChat (413): shorten the conversation or attachments.",
    422: "GigaChat rejected the request body (422, no details returned). Most often the conversation history contains tool-calling turns GigaChat cannot re-validate in its legacy functions format (one function per request, strict function_call -> function pairing). Start a fresh session for this model, or drop the incompatible turns.",
    429: "GigaChat kept answering 429 (rate limit) after retries — the plan's concurrency limit is saturated.",
    502: "The upstream call kept failing after retries."
  };
  const hint = hints[status] ?? `GigaChat returned HTTP ${status} with an empty body.`;
  return { kind: "raw", status, raw: Buffer.from(JSON.stringify({ error: { message: `dsh-llm-gigachat: ${hint}`, type: "upstream_error", code: status } })), contentType: "application/json" };
}
 ...
 *   stripToolsFor         — model ids whose requests have tools stripped entirely
 *                           (weaker models: they reject complex schemas with 422
 *                           or hallucinate tool calls)
...
export function createGigaChatProxy(options = {}) {
  ...
  async function fetchToken(overrideCredentials) {
    const credentials = overrideCredentials ?? (typeof resolveCredentials === "function" ? await resolveCredentials() : undefined);
    if (!credentials) {
  ...
  async function getToken(force = false, overrideCredentials) {
    if (!force && !overrideCredentials && cachedToken && Date.now() + TOKEN_MARGIN_MS < cachedToken.expiresAtMs) return cachedToken.token;
    if (overrideCredentials) {
      const t = await fetchToken(overrideCredentials);
      return t.token;
    }
  ...
  /** GET one upstream path with a Bearer token, resolved to its body text. */
  function upstreamGetText(base, path, token) {
    return new Promise((resolve, reject) => {
      const mod = base.startsWith("https:") ? https : http;
      const req = mod.request(base + path, {
        method: "GET",
        headers: { "Authorization": `Bearer ${token}`, "Accept": "application/json" },
        rejectUnauthorized,
      }, (up) => { readUpstreamText(up).then(resolve); });
      req.on("error", reject);
      req.end();
    });
  }
  ...
  /**
   * GET /v1/models: pass through to the real GigaChat API (chat models only —
   * embedders are useless in the chat picker). A Bearer credential supplied in
   * the incoming Authorization header (the Models page "Fetch available models"
   * probe) is honored first, so fetch works even before the key is saved.
   * Any failure falls back to the configured catalog so the endpoint stays up.
   */
  async function serveModelsEndpoint(req, res) {
    const authHeader = req.headers.authorization ?? "";
    const overrideCred = authHeader.startsWith("Bearer ") ? authHeader.slice(7).trim() : void 0;
    const staticList = () => JSON.stringify({
      object: "list",
      data: models.map((m) => ({ id: m.id, object: "model", owned_by: "sber", display_name: m.name ?? m.id })),
    });
    try {
      const token = await getToken(false, overrideCred && overrideCred.length > 0 ? overrideCred : void 0);
      const text = await upstreamGetText(upstreamBaseURL, "/models", token);
      const parsed = JSON.parse(text);
      if (!Array.isArray(parsed?.data)) throw new Error("unexpected /models payload");
      const data = parsed.data
        .filter((m) => m && (m.type === void 0 || m.type === "chat"))
        .map((m) => ({
          id: m.id,
          object: "model",
          owned_by: m.owned_by ?? "sber",
          display_name: (models.find((x) => x.id === m.id) ?? {}).name ?? KNOWN_MODEL_NAMES[m.id] ?? m.id,
        }));
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ object: "list", data }));
      logInfo(`models passthrough: ${data.length} chat models from upstream`);
    } catch (err) {
      logInfo(`models passthrough failed (${err.message}) — serving configured catalog`);
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(staticList());
    }
  }
  ...
  server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (Array.isArray(parsed.tools) && parsed.tools.length > 0 && parsed.model && stripToolsFor.includes(parsed.model)) {
        delete parsed.tools;
        delete parsed.tool_choice;
        logInfo(`tools stripped for model "${parsed.model}" (stripToolsFor)`);
      }
      ...
      const result = await enqueueUpstream(async () => {
        ...
        while (attempts-- > 0) {
          ...
          if (status !== 200) {
            ...
            return friendlyUpstreamError(status, text);
          }
        ...
        return friendlyUpstreamError(lastFailure?.status ?? 502, lastFailure?.text ?? "");
      });
