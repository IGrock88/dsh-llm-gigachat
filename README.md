...
| `stripToolsFor` | `[]` | id моделей, у которых запросы идут **без инструментов** — для слабых моделей, которые отвергают сложные агентские схемы (422) или галлюцинируют вызовы |
...
- **Рекомендации по моделям**: для агентских чатов (с инструментами) используйте `GigaChat-3-Ultra` / `GigaChat-3-Pro` — младшие слабо следуют схемам и могут вызывать инструменты невпопад; для них включите `stripToolsFor` (тогда модель отвечает текстом, без tools).
